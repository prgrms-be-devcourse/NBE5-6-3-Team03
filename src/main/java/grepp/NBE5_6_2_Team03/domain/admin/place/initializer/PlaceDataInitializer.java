package grepp.NBE5_6_2_Team03.domain.admin.place.initializer;
...
public class PlaceDataInitializer implements CommandLineRunner {
    ...
    public void run(String[] args) {
        ...
        if (!dataInitialized) {  // 초기화되지 않았다면
            List<CityDto> cities = cityService.getAllCities(); // 모든 도시 조회

            for (CityDto cityDto : cities) {
                Set<String> placeIds = new HashSet<>();
                placeIds.addAll(googlePlaceService.searchPlaceIds(cityDto.getLatitude(), cityDto.getLongitude(), cityDto.getRadius(), "tourist_attraction", 2));
                placeIds.addAll(googlePlaceService.searchPlaceIds(cityDto.getLatitude(), cityDto.getLongitude(), cityDto.getRadius(), "point_of_interest", 2));
